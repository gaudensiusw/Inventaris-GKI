...
<body class="bg-slate-50 min-h-screen flex items-center justify-center p-4">
    <!-- Back to Home Button -->
    <a href="{{ route('home') }}"
        class="fixed top-8 left-8 flex items-center gap-2 px-5 py-2.5 bg-white text-slate-600 rounded-2xl text-xs font-black uppercase shadow-xl shadow-slate-200/50 hover:bg-slate-50 hover:scale-105 transition-all z-50">
        <i data-lucide="arrow-left" class="w-4 h-4"></i>
        <span>Kembali ke Beranda</span>
    </a>

    <div
        class="max-w-6xl w-full grid grid-cols-1 lg:grid-cols-2 bg-white rounded-[40px] shadow-2xl overflow-hidden border border-slate-100">
        ...
        <div class="relative bg-gradient-premium p-12 lg:p-20 flex flex-col justify-between overflow-hidden">
            ...
            <div class="relative z-10">
                <div
                    class="flex items-center gap-3 bg-white/5 w-fit px-4 py-2 rounded-2xl backdrop-blur-md border border-white/10 mb-12">
                    <div
                        class="w-8 h-8 bg-blue-600 rounded-xl flex items-center justify-center text-white font-black text-sm shadow-lg shadow-blue-500/20">
                        GKI</div>
                ...
                <p class="text-slate-400 text-lg max-w-md leading-relaxed font-medium">
                    Kelola aset, peminjaman, dan perbaikan barang gereja secara profesional dalam satu dashboard
                    terintegrasi.
                </p>
            </div>
        ...
        <div class="p-12 lg:p-20 flex flex-col justify-center">
            <div class="max-w-md mx-auto w-full">
                <div class="flex flex-col gap-3 mb-12">
                    <div
                        class="w-16 h-16 bg-slate-50 text-slate-800 rounded-2xl flex items-center justify-center mb-4 border border-slate-100 shadow-sm">
                ...
                <form action="{{ route('login') }}" method="POST" class="space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative group">
                            <i data-lucide="mail"
                                class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 group-focus-within:text-slate-800 transition-colors"></i>
                            <input type="email" name="email" value="{{ old('email') }}"
                                placeholder="admin@gkidelima.org" required autofocus
                        ...
                        @error('email') <span class="text-xs text-red-500 font-bold ml-1">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <button type="submit"
                        class="w-full py-5 bg-slate-900 text-white rounded-2xl font-black uppercase text-sm shadow-2xl shadow-slate-200 hover:bg-black active:scale-[0.98] transition-all">
                        Masuk Dashboard
                    </button>
                </form>
