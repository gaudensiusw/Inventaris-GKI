...
<div class="flex flex-col gap-8">
    ...
    <div class="flex justify-between items-center">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('borrowing.requests') }}" class="px-5 py-3 bg-amber-50 text-amber-700 rounded-2xl text-xs font-black uppercase border border-amber-200 hover:bg-amber-100 transition-all flex items-center gap-2">
                <i data-lucide="inbox" class="w-4 h-4"></i>
                Request Masuk
                @if(($stats['pending_requests'] ?? 0) > 0)
                    <span class="ml-1 px-2 py-0.5 bg-amber-500 text-white rounded-full text-[10px] font-bold">{{ $stats['pending_requests'] }}</span>
                @endif
            </a>
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-right">
                            <button onclick="openReturnModal({{ $borrow->id_pinjam }}, '{{ addslashes($borrow->item->name ?? 'Barang') }}', {{ $borrow->qty }})" 
                                class="px-4 py-2 bg-emerald-50 text-emerald-600 rounded-xl text-[10px] font-black uppercase hover:bg-emerald-600 hover:text-white transition-all">
                                Kembalikan
                            </button>
                        </td>
                    </tr>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        itemSelect.on('change', (value) => {
            ...
            if (value) {
                qtyInput.max = stock;
                stockHint.innerText = `Maksimal pinjam: ${stock} unit`;
                stockHint.classList.replace('text-slate-400', 'text-blue-500');
                ...
                stockHint.classList.replace('text-blue-500', 'text-slate-400');
            }
        });
    });
    ...
    @endif
    function openReturnModal(id, itemName, maxQty) {
        document.getElementById('returnForm').action = '/admin/borrowing/' + id + '/return';
        document.getElementById('returnItemName').innerText = itemName;
        document.getElementById('returnMaxQty').innerText = maxQty;
        document.getElementById('returnQtyInput').max = maxQty;
        document.getElementById('returnQtyInput').value = maxQty;
...
<div id="returnModal" class="fixed inset-0 z-[100] hidden overflow-y-auto">
    <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm" onclick="closeModal('returnModal')"></div>
    <div class="flex min-h-full items-center justify-center p-4">
        <div class="relative bg-white rounded-[40px] w-full max-w-md overflow-hidden shadow-2xl transform transition-all scale-95 opacity-0 duration-300" id="returnModalContent">
            <div class="px-8 py-6 bg-emerald-50 border-b border-emerald-100 flex justify-between items-center">
                <h2 class="text-xl font-black text-slate-800">Pengembalian Barang</h2>
                <button onclick="closeModal('returnModal')" class="p-2 hover:bg-white rounded-xl transition-all">
                    <i data-lucide="x" class="w-5 h-5 text-slate-400"></i>
                </button>
            </div>
            <form id="returnForm" method="POST" class="p-8">
                @csrf
                <div class="mb-4 p-4 bg-slate-50 rounded-2xl">
                    <p class="text-xs text-slate-400 font-bold">Barang</p>
                    <p id="returnItemName" class="text-sm font-black text-slate-800"></p>
                    <p class="text-xs text-slate-400 mt-1">Qty dipinjam: <strong id="returnMaxQty" class="text-blue-600"></strong> unit</p>
                </div>
                <div class="space-y-4">
                    <div>
                        <label class="block text-xs font-bold text-slate-500 mb-2">Jumlah Dikembalikan *</label>
                        <input type="number" name="qty_kembali" id="returnQtyInput" required min="1"
                            class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold text-blue-600 focus:ring-4 focus:ring-blue-50 focus:border-blue-400">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-500 mb-2">Kondisi Barang *</label>
                        <select name="kondisi_kembali" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-medium focus:ring-4 focus:ring-blue-50 focus:border-blue-400">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-500 mb-2">Catatan Pengembalian</label>
                        <textarea name="catatan_kembali" rows="2" placeholder="Catatan kondisi barang saat dikembalikan..."
                            class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-medium focus:ring-4 focus:ring-blue-50 focus:border-blue-400 resize-none"></textarea>
                    </div>
                </div>
                <div class="mt-6 flex gap-3">
                    <button type="button" onclick="closeModal('returnModal')" class="flex-1 px-6 py-4 bg-slate-100 text-slate-600 rounded-2xl text-xs font-black uppercase">Batal</button>
                    <button type="submit" class="flex-[2] px-6 py-4 bg-emerald-600 text-white rounded-2xl text-xs font-black uppercase shadow-xl shadow-emerald-200 hover:bg-emerald-700 transition-all">Konfirmasi Pengembalian</button>
                </div>
            </form>
        </div>
    </div>
