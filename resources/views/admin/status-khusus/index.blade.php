...
@section('content')
    <div class="flex flex-col gap-8">
        ...
        <div class="flex gap-2 bg-slate-100/50 p-1 rounded-2xl w-fit">
            <a href="{{ route('special-status.index', ['tab' => 'hilang']) }}"
                class="px-6 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center gap-2 {{ $tab == 'hilang' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700' }}">
                <i data-lucide="clock" class="w-4 h-4"></i>
                Hilang ({{ $stats['hilang'] }})
            </a>
            <a href="{{ route('special-status.index', ['tab' => 'tidak-digunakan']) }}"
                class="px-6 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center gap-2 {{ $tab == 'tidak-digunakan' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700' }}">
                <i data-lucide="eye-off" class="w-4 h-4"></i>
                Tidak Digunakan ({{ $stats['tidak_digunakan'] }})
            </a>
            <a href="{{ route('special-status.index', ['tab' => 'pengadaan']) }}"
                class="px-6 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center gap-2 {{ $tab == 'pengadaan' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700' }}">
                        ...
                        <div class="flex justify-between items-start">
                            <div class="flex flex-col gap-1">
                                <h3 class="text-xl font-black text-slate-800">{{ $item->name }}
                                    <span class="text-xs font-bold text-slate-300 ml-2 flex items-center gap-2 inline-flex">
                                        ...
                                        <span class="relative group/qr cursor-pointer">
                                            ...
                                            <div
                                                class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover/qr:block z-50 animate-in fade-in zoom-in duration-200">
                                ...
                                <p class="text-xs font-bold text-slate-400 uppercase tracking-widest">
                                    {{ $item->category->name ?? 'Tanpa Kategori' }}</p>
                            </div>
                            <div class="flex gap-2">
                                <button
                                    class="p-2.5 bg-slate-50 text-slate-400 rounded-xl hover:bg-blue-50 hover:text-blue-600 transition-all"><i
                                        data-lucide="edit-3" class="w-5 h-5"></i></button>
                            </div>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 pt-4 border-t border-slate-50">
                            <div class="flex flex-col gap-1">
                                <span class="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Lokasi
                                    Terakhir</span>
                            ...
                            <div class="flex flex-col gap-1">
                                ...
                                <span class="text-sm font-bold text-slate-700">
                                    {{ $tab == 'hilang' ? $item->qty_hilang : ($tab == 'tidak-digunakan' ? $item->qty_tidak_digunakan : $item->qty_pengadaan) }}
                                    unit
                                </span>
                            </div>
                            <div class="flex flex-col gap-1">
                                <span class="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Update
                                    Terakhir</span>
                                <span class="text-sm font-bold text-slate-700">{{ $item->updated_at->format('d M Y') }}</span>
                            </div>
                            <div class="flex flex-col gap-1">
                                <span class="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Tanggal
                                    Beli</span>
                                <span
                                    class="text-sm font-bold text-slate-700">{{ $item->purchase_date ? \Carbon\Carbon::parse($item->purchase_date)->format('d M Y') : '-' }}</span>
                            </div>
                        </div>
            ...
            @if($items->hasPages() || $items->total() > 10)
                <div class="flex justify-between items-center bg-white p-6 rounded-[32px] shadow-sm border border-slate-100">
                    <div class="flex items-center gap-4">
                        ...
                        <select onchange="window.location.href = addQueryParam('per_page', this.value)"
                            class="px-3 py-1.5 bg-slate-50 border border-slate-100 rounded-xl text-xs font-bold text-slate-600 focus:outline-none">
